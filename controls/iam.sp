    ...
    select
      ...
      case
        when
          count(grantee_name) < 2
        then
          'ACCOUNTADMIN role is granted to less than 2 users.'
        when
          count(grantee_name) > 2
        then
          'ACCOUNTADMIN role is granted to more than 2 users.'
        else
          'ACCOUNTADMIN privilage is granted to two users.'
      end
      as reason
    from
...
control "iam_user_with_accountadmin_role_have_email" {
  ...
  sql = <<EOT
    ...
    select
      ...
      case
        when email != '' then name || ' have email address set.'
        else name || ' does not have email address set.'
      end as reason
    from
      snowflake_user as su inner join users_with_account_admin_role as sua on su.name = sua.grantee_name
    EOT
}
